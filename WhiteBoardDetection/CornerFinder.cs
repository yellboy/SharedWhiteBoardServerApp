...
{
    public class CornerFinder : ICornerFinder
    {
        private readonly ISimilarityChecker _similarityChecker;
        private readonly IImageRotator _imageRotator;

        public CornerFinder(ISimilarityChecker similarityChecker, IImageRotator imageRotator)
        {
            _similarityChecker = similarityChecker;
            _imageRotator = imageRotator;
        }

        public WhiteBoardCorners Find(Bitmap image, Bitmap template1, Bitmap template2, Bitmap template3, Bitmap template4, IReadOnlyCollection<RectangularContour> rectangles)
        {
